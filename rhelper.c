...
static void call_lua_post_interface_renderer(BITMAP *scx, int interface_vs) {
    const char *rfname = "sys_render_post_interface";
    ...
    con_h = ((int)gfxctl.console_lineheight) * ((int)gfxctl.console_lines);
    wake_x = gd.vxo + 224;
    wake_y = gd.vyo + gd.vyo_off + 122;

    lua_pushinteger(LUA, interface_vs);
    lua_pushinteger(LUA, gfxctl.con_x);
    lua_pushinteger(LUA, gfxctl.con_y);
    lua_pushinteger(LUA, 640);
    lua_pushinteger(LUA, con_h);
    lua_pushinteger(LUA, wake_x);
    lua_pushinteger(LUA, wake_y);

    lc_call_topstack(8, rfname);
}
...
int draw_interface(int softmode, int interface_vs, int really) {
    ...
    /* 既存mod影響回避2: 実描画時に処理をする。二重描画や副作用（状態更新が2回走る等）を防ぐ。未定義なら何もしない */
    call_lua_post_interface_renderer(scx, interface_vs);
